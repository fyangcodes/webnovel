...
class TranslationPermissionMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        messages.error(self.request, "You don't have permission to perform translation tasks.")
        return redirect('books:book_list')
...
class EditorPermissionMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        messages.error(self.request, "You need editor permissions to perform this action.")
        return redirect('books:book_list')
...
class AdminPermissionMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        messages.error(self.request, "You need administrator permissions to perform this action.")
        return redirect('books:book_list')
...
class WriterPermissionMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        messages.error(self.request, "You need writer permissions to perform this action.")
        return redirect('books:book_list')
...
class RoleRequiredMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        role_names = [dict(User.ROLE_CHOICES).get(role, role) for role in self.required_roles]
        messages.error(self.request, f"You need one of these roles: {', '.join(role_names)}")
        return redirect('books:book_list') 