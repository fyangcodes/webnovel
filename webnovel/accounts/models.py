...
class BookCollaborator(models.Model):
    ...
    book = models.ForeignKey('books.Book', on_delete=models.CASCADE, related_name='collaborators')
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='book_collaborations')
...
class TranslationAssignment(models.Model):
    ...
    chapter = models.ForeignKey('books.Chapter', on_delete=models.CASCADE, related_name='translation_assignments')
    translator = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='translation_assignments')
    target_language = models.ForeignKey('books.Language', on_delete=models.CASCADE)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    assigned_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='assigned_translations')
