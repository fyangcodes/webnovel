...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--models',
            nargs='+',
            help='Specific models to backup (default: all)'
        )
        parser.add_argument(
            '--compress',
            action='store_true',
    ...
    def backup_database(self, backup_path, specific_models=None):
        ...
        from books.models import Language, Author, Book, Chapter, ChapterImage, BookFile, ChangeLog
        from accounts.models import User, BookCollaborator, TranslationAssignment
        ...
        if specific_models:
            ...
            for model, app in all_models:
                ...
                if model_name in specific_models or model._meta.model_name in specific_models:
        ...
        for model, app in all_models:
            try:
                ...
                if count == 0:
                    self.stdout.write(f"   ⏭️  Skipping {model._meta.model_name} (no records)")
                    continue
                
                filename = f"{backup_path}/db_{app}_{model._meta.model_name}.json"
                with open(filename, 'w', encoding='utf-8') as f:
                    serializers.serialize('json', model.objects.all(), stream=f, indent=2)
                ...
                self.stdout.write(
                    self.style.ERROR(f"   ❌ Error backing up {model._meta.model_name}: {str(e)}")
                )
    ...
    def backup_files(self, backup_path):
        ...
        self.stdout.write(f"   📈 Total files: {total_files} ({self.format_size(total_size)})")
            ...
            for root, dirs, files in default_storage.walk(source_path):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        self.stdout.write(
                            self.style.WARNING(f"   ⚠️  Could not copy {source_file}: {str(e)}")
                        )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f"   ❌ Error walking directory {source_path}: {str(e)}")
            )
...
'''
    ...
    def compress_backup(self, backup_path):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING("   ⚠️  tarfile module not available, skipping compression")
            )
        except Exception as e:
            self.stdout.write(
