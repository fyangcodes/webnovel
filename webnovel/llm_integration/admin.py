...
class LLMProviderAdmin(admin.ModelAdmin):
    list_display = ['name', 'display_name', 'is_active', 'api_key_configured', 'default_model', 'cost_per_1k_tokens']
...
class LLMServiceCallAdmin(admin.ModelAdmin):
    list_display = ['provider', 'model_name', 'operation', 'status', 'response_time_ms', 'total_tokens', 'created_at']
    ...
    fieldsets = (
        ...
        ('Response Details', {
            'fields': ('response_time_ms', 'error_message')
        }),
        ('Content Context', {
            'fields': ('book', 'chapter', 'source_language', 'target_language')
        }),
        ('Quality Metrics', {
            'fields': ('quality_score', 'user_feedback')
        }),
        ('Metadata', {
...
class LLMQualityMetricsAdmin(admin.ModelAdmin):
    list_display = ['provider', 'model_name', 'operation', 'total_calls', 'success_rate', 'avg_response_time_ms', 'period_start']
    ...
    fieldsets = (
        ('Provider Information', {
            'fields': ('provider', 'model_name', 'operation')
        }),
        ('Metrics', {
            'fields': ('total_calls', 'success_rate', 'avg_response_time_ms', 'avg_quality_score')
        }),
        ('Usage & Cost', {
